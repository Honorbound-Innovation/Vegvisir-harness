...
impl<M: Model> SubAgentSupervisor<M> {
    ...
    fn load_board(&mut self) -> anyhow::Result<()> {
        ...
        self.board = crate::tools::load_subagent_board_records(path)?
            .into_iter()
    ...
    fn save_board(&self) -> anyhow::Result<()> {
        ...
        let records = self.board.values().cloned().collect::<Vec<_>>();
        crate::tools::save_subagent_board_records(path, &records)
    }
