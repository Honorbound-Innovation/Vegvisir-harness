...
- Tool-call round limit: tool calls are unlimited by default. `/tool-limit` shows the current maximum tool-call rounds per model turn, `/tool-limit <rounds>` sets a cap for the running session, and `/tool-limit unlimited` or `/tool-limit default` clears the runtime cap back to the environment value or unlimited. `/tools max-rounds <rounds>` is an equivalent shortcut.
