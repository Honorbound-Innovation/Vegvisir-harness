...
- Agent Context Protocol: when `AGENT.md` or `agent/` exists, Vegvisir supplies bounded ACP project context automatically. Use `/acp status`, `/acp validate`, `/acp context`, and `/acp run <name>` to inspect or run workspace command documents. ACP Markdown is context, not executable shell code, and never overrides Vegvisir safety or approval policy.
