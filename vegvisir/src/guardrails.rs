...
struct ApprovalLedgerState {
    ...
    pub approved_patterns: BTreeSet<String>,
}
...
impl ApprovalLedger {
    ...
    pub fn approve_pattern(&mut self, id: &str) -> Option<String> {
        let tool = self.state.lock().ok().and_then(|mut state| {
            let tool = state.pending.get(id)?.tool_name.clone();
            state.approved_patterns.insert(tool.clone());
            Some(tool)
        });
        self.save();
        tool
    ...
    pub fn consume_approval(&mut self, id: &str, tool_name: &str) -> bool {
        let mut consumed = false;
        if let Ok(mut state) = self.state.lock() {
            if state.approved_patterns.contains(tool_name) {
...
impl GuardrailEngine {
    pub fn authorize_tool(&mut self, tool: &Tool, args: &Map<String, Value>) -> anyhow::Result<()> {
        ...
        if self.policy.require_human_approval && tool.risky {
            let request_id = approval_request_id(&tool.name, args);
            if self.approvals.consume_approval(&request_id, &tool.name) {
