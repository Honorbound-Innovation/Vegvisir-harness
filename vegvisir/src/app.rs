...
impl TuiApplication {
    ...
    pub fn run(&mut self) -> anyhow::Result<()> {
        self.run_terminal_transcript()
    }

    pub fn run_fullscreen(&mut self) -> anyhow::Result<()> {
    ...
    fn run_terminal_transcript(&mut self) -> anyhow::Result<()> {
        let mut terminal = TranscriptTerminal::enter()?;
        terminal.print_startup(self)?;
        let mut printed_messages = self.session.messages.len();
        let mut streaming_assistant: Option<(usize, usize)> = None;
        terminal.draw_prompt(self)?;
        self.redraw_requested = false;
        ...
        while self.running {
            if event::poll(Duration::from_millis(50))? {
                ...
                terminal.print_new_messages(self, &mut printed_messages, streaming_assistant)?;
                terminal.draw_prompt(self)?;
            }

            let before_stream = streaming_assistant;
            self.poll_stream_events();
            if self.redraw_requested || before_stream != streaming_assistant {
                terminal.clear_prompt()?;
                streaming_assistant = terminal.print_stream_delta(self, streaming_assistant)?;
                terminal.draw_prompt(self)?;
                self.redraw_requested = false;
            }

            let completed = self.poll_pending_send();
            let background_changed = self.poll_background_jobs();
            if completed || background_changed {
                terminal.clear_prompt()?;
                if let Some((assistant_index, _)) = streaming_assistant.take() {
                    terminal.finish_stream_line()?;
                    printed_messages = printed_messages.max(assistant_index + 1);
                }
                terminal.print_new_messages(self, &mut printed_messages, streaming_assistant)?;
                terminal.draw_prompt(self)?;
                self.redraw_requested = false;
...
impl TranscriptTerminal {
    ...
    fn print_startup(&mut self, app: &TuiApplication) -> anyhow::Result<()> {
        ...
        if app.session.messages.is_empty() {
            ...
            for message in &app.session.messages {
                print_transcript_message(&mut stdout, message, width)?;
            }
        }
        stdout.flush()?;
        Ok(())
    ...
    fn clear_prompt(&mut self) -> anyhow::Result<()> {
        ...
        if self.prompt_lines > 1 {
            execute!(stdout, MoveUp((self.prompt_lines - 1) as u16))?;
        }
    ...
    fn draw_prompt(&mut self, app: &TuiApplication) -> anyhow::Result<()> {
        let mut stdout = io::stdout();
        let width = transcript_width();
        let input_width = width.saturating_sub(2).max(1);
        let mut lines = transcript_input_lines(&app.input.buffer, input_width);
        if lines.is_empty() {
            lines.push(String::new());
        }
        let visible_lines = lines.len().min(6);
        let hidden = lines.len().saturating_sub(visible_lines);
        let visible = &lines[hidden..];
        ...
        let autocomplete_lines = if app.input.buffer.starts_with('/') {
            app.input.suggestions.len().min(5)
        } else {
            0
        };
        self.prompt_lines =
            visible_lines + autocomplete_lines + usize::from(autocomplete_lines > 0);
        let (cursor_line, cursor_column) = app.input.visual_cursor_position(input_width);
        let visible_cursor_line = cursor_line
            .saturating_sub(hidden)
            .min(visible_lines.saturating_sub(1));
        if self.prompt_lines > 1 {
            execute!(
                stdout,
                MoveUp((self.prompt_lines - 1 - visible_cursor_line) as u16)
            )?;
        }
        execute!(
            stdout,
            MoveToColumn((2 + cursor_column.min(input_width)) as u16)
        )?;
        stdout.flush()?;
        Ok(())
    }
    ...
    fn print_new_messages(
        ...
        streaming_assistant: Option<(usize, usize)>,
    ) -> anyhow::Result<()> {
        let mut stdout = io::stdout();
        let width = transcript_width();
        while *printed_messages < app.session.messages.len() {
            let index = *printed_messages;
            ...
            if streaming_assistant
                .map(|(assistant_index, _)| assistant_index == index)
                .unwrap_or(false)
            {
                *printed_messages += 1;
                continue;
    ...
    fn print_stream_delta(
        ...
        let Some(index) = app
            ...
            return Ok(streaming_assistant);
        };
        let content = &app.session.messages[index].content;
        let previous_len = streaming_assistant
            .filter(|(assistant_index, _)| *assistant_index == index)
            .map(|(_, len)| len)
            .unwrap_or(0);
...
impl Drop for TranscriptTerminal {
    fn drop(&mut self) {
...
fn print_transcript_message(
    ...
    let label = match message.role.as_str() {
        "user" => ansi_info("you"),
        "assistant" => ansi_success("agent"),
        "system" => ansi_warning("note"),
        other => ansi_dim(other),
    };
    let available = width.saturating_sub(8).max(20);
    let mut first = true;
    for logical in message.content.lines() {
...
fn transcript_input_lines(text: &str, width: usize) -> Vec<String> {
    ...
    for logical in text.split('\n') {
        let wrapped = transcript_wrap(logical, width);
        if wrapped.is_empty() {
            lines.push(String::new());
        } else {
...
fn transcript_wrap(text: &str, width: usize) -> Vec<String> {
...
fn ansi_info(text: &str) -> String {
    format!("\x1b[36m{text}\x1b[0m")
}

fn ansi_success(text: &str) -> String {
    format!("\x1b[32m{text}\x1b[0m")
}

fn ansi_warning(text: &str) -> String {
