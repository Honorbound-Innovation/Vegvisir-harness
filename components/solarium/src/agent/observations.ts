...
export class ObservationRecorder {
  ...
  async observe(options: ObservationOptions = {}): Promise<PageObservation> {
    ...
    const domObservation = (await this.page.evaluate(
      ({ maxTextChars, maxElements, redactInputValues, sensitiveSelectors, redactedValue }) => {
        ...
        const cssEscape = (value: string): string => CSS.escape(String(value));
        ...
        const attrText = (input: Element): string => [
        ...
        const matchesAnySensitiveSelector = (input: Element): boolean =>
          sensitiveSelectors.some((selector: string) => {
            try {
        ...
        const shouldRedactInput = (input: HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement): boolean => {
        ...
        const boundingBoxFor = (element: Element) => {
          const rect = element.getBoundingClientRect();
          if (!Number.isFinite(rect.width) || !Number.isFinite(rect.height)) return null;
          return { x: rect.x, y: rect.y, width: rect.width, height: rect.height };
        };

        const visibleFor = (element: Element) => {
          const rect = element.getBoundingClientRect();
        ...
        const selectorHintsFor = (element: Element) => {
          const hints: Array<{
            kind: string;
            strategy: "css" | "playwright";
            value: string;
          ...
          const label = labelTextFor(element);
          if (label) {
            hints.push({ kind: "label", strategy: "playwright", value: `getByLabel(${JSON.stringify(label)})`, confidence: "high", reason: "Associated label text." });
          }
        ...
        const buttons = Array.from(
          document.querySelectorAll<HTMLButtonElement | HTMLInputElement>("button, input[type='button'], input[type='submit'], input[type='reset']")
        )
          .slice(0, maxElements)
          .map((button) => ({
            ...metadataFor(button),
            text: cleanText(
              button instanceof HTMLInputElement ? button.value || button.getAttribute("aria-label") || button.name : button.innerText || button.getAttribute("aria-label")
            ),
        ...
        const inputs = Array.from(document.querySelectorAll("input, textarea, select"))
          .slice(0, maxElements)
          .map((input) => mapInput(input as HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement));
        ...
        const landmarks = Array.from(
          document.querySelectorAll(
            "main, nav, header, footer, aside, section[aria-label], section[aria-labelledby], [role='main'], [role='navigation'], [role='banner'], [role='contentinfo'], [role='complementary'], [role='region'], dialog, [role='dialog'], ul, ol"
          )
        )
          .slice(0, maxElements)
          .map((element) => {
            ...
            return {
              ...
              tagName: tag,
              text: cleanText((element as HTMLElement).innerText || element.textContent).slice(0, 500)
            };
          });
        ...
        const tables = Array.from(document.querySelectorAll("table"))
          ...
          .map((table) => ({
            ...
            sampleRows: Array.from(table.querySelectorAll("tr"))
              .slice(0, 5)
              .map((row) => Array.from(row.querySelectorAll("th,td")).slice(0, 10).map((cell) => cleanText(cell.textContent)))
          }));
      ...
      { maxTextChars, maxElements, redactInputValues, sensitiveSelectors, redactedValue: REDACTED_VALUE }
